// ===========================================================================
// mod/trivia.js — Trivia as an SPA module. mount(root, ctx) / unmount().
// Same Firebase data model (trivia/{code}) and leaderboard as before.
// ===========================================================================
...
const SCREENS = `
  ...
  <section id="screen-join" class="screen active">
    <div class="wrap">
      <div class="brandbar"><div class="logo">LS</div><div><h1>Trivia</h1><p class="tagline">A Legal Soft game</p></div></div>
      <div class="card">
        ...
        <div class="field"><label for="opt-cat">Category</label>
          <select id="opt-cat">
            ...
            <option value="Fun &amp; Pop Culture">Fun &amp; Pop Culture</option>
        ...
        <div class="field"><label for="opt-level">Difficulty</label>
          <select id="opt-level">
        ...
        <div class="row">
          ...
          <div class="field" style="flex:1"><label for="opt-time">Seconds each</label>
  ...
  <section id="screen-lobby" class="screen">
    <div class="wrap">
      <div class="brandbar"><div class="logo">LS</div><div><h1>Trivia</h1><p class="tagline" id="lobby-sub">Lobby</p></div></div>
      <div class="card">
  ...
  <section id="screen-done" class="screen">
    <div class="wrap">
      <div class="brandbar"><div class="logo">&#127942;</div><div><h1>Final results</h1><p class="tagline">Trivia</p></div></div>
      <div class="card">
...
// ---- misc -----------------------------------------------------------------
$("btn-copy").addEventListener("click", async () => { const url = `${location.origin}${location.pathname}#/trivia?room=${ROOM}`; try { await navigator.clipboard.writeText(url); $("btn-copy").textContent = "Copied!"; } catch { prompt("Invite link:", url); } setTimeout(() => ($("btn-copy").textContent = "Copy invite link"), 1500); });
$("btn-home").addEventListener("click", () => ctx.goHome());
  $("btn-leave").addEventListener("click", async () => { try { if (ROOM && ME) await update(ref(db, `trivia/${ROOM}/players/${ME}`), { connected: false }); } catch {} ctx.goHome(); });
...
const _hq = (location.hash.split("?")[1]) || ""; const params = new URLSearchParams(_hq);
  if (params.get("room")) $("join-code").value = params.get("room").toUpperCase();


  _detach = detach; _stop = stopHostTimer;
}
