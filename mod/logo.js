// ===========================================================================
// mod/logo.js — logo as an SPA module (mount/unmount). Same Firebase model.
// ===========================================================================
// ===========================================================================
...
const Sound = {
  ...
  round() { Audio.sfx("round"); },
  tap() { Audio.sfx("tap"); },
  win() { Audio.sfx("correct"); },
  zoom() { Audio.sfx("reveal"); },
  end() { Audio.celebrate(); },
};
...
const SCREENS = `
  ...
  <section id="screen-join" class="screen active">
    <div class="wrap">
      <div class="brandbar"><div class="logo">LS</div><div><h1>Guess the Logo</h1><p class="tagline">A Legal Soft game</p></div></div>
      <div class="card">
        ...
        <div class="field"><label for="opt-cat">Category</label>
          <select id="opt-cat">
            <option value="mixed" selected>Mixed (all categories)</option>
        ...
        <div class="field"><label for="opt-count">Number of logos</label>
          <select id="opt-count"><option value="5">5</option><option value="10" selected>10</option><option value="15">15</option><option value="20">20</option></select>
        </div>
  ...
  <section id="screen-lobby" class="screen">
    <div class="wrap">
      <div class="brandbar"><div class="logo">LS</div><div><h1>Guess the Logo</h1><p class="tagline" id="lobby-sub">Lobby</p></div></div>
      <div class="card">
  ...
  <section id="screen-game" class="screen">
    <div class="game-shell">
      ...
      <div class="logo-stage">
        <div class="logo-viewport"><img id="logo-img" alt="logo" /></div>
      </div>
      <div class="stage-info" id="stage-info"></div>
      <p class="status-line" id="status-line"></p>
      <div class="host-answer" id="host-answer" style="display:none"></div>

      <div id="mini-board" class="scoreboard" style="display:none"></div>

      <div class="chat-log" id="chat-log"></div>
      <div class="answer-row" id="answer-row" style="display:none">
        <input id="answer-input" placeholder="Type your guess…" autocomplete="off" />
        <button id="answer-send" class="btn-primary">Guess</button>
      </div>

      <div id="host-controls" class="host-controls" style="display:none">
        <button id="btn-zoom" class="btn-ghost">Zoom out</button>
        <button id="btn-next" class="btn-primary" style="display:none">Next logo</button>
      </div>

      <a class="attrib" href="https://logo.dev" target="_blank" rel="noopener">Logos provided by Logo.dev</a>
  ...
  <section id="screen-done" class="screen">
    <div class="wrap">
      <div class="brandbar"><div class="logo">&#127942;</div><div><h1>Final results</h1><p class="tagline">Guess the Logo</p></div></div>
      <div class="card">
...
export function mount(root, __shell) {
  if (!document.getElementById("logo-css")) { const _l = document.createElement("link"); _l.rel = "stylesheet"; _l.href = "logo/styles.css"; _l.id = "logo-css"; document.head.appendChild(_l); }
...
function onMeta() {
  ...
  if (meta.state === "playing" || meta.state === "revealed") Music.start(); else if (meta.state === "lobby") Music.stop();
}
function onPlayers() {
  if (!meta) return;
...
// ---- misc -----------------------------------------------------------------
$("btn-copy").addEventListener("click", async () => { const url = `${location.origin}${location.pathname}#/logo?room=${ROOM}`; try { await navigator.clipboard.writeText(url); $("btn-copy").textContent = "Copied!"; } catch { prompt("Invite link:", url); } setTimeout(() => ($("btn-copy").textContent = "Copy invite link"), 1500); });

$("btn-leave").addEventListener("click", async () => { try { if (ROOM && ME) await update(ref(db, `logo/${ROOM}/players/${ME}`), { connected: false }); } catch {} __shell.goHome(); });
...
export function unmount() {
  ...
  _detach(); _stop();
  Music.stop();
  document.getElementById("logo-css")?.remove();
}
